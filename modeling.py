...
with mlflow.start_run(run_name="LGBM_5Fold_Calibrated"):
    ...
    # ═══════════════════════════════════════════════════════════════
    header("STEP 5 — Model Calibration (Isotonic Regression - Unleaked)")
    
    step("Fitting Isotonic Regression on strict OOF predictions...")
    # FIX: By fitting IsotonicRegression directly on oof_preds vs y, we guarantee NO LEAKAGE
    # because each oof_preds[i] was generated by a model that didn't see X[i].
    ...
    log(f"Brier BEFORE cal: {brier:.4f}")
    log(f"Brier AFTER  cal: {brier_cal:.4f}")
    ...
    metrics = {
        ...
        'brier_calibrated': float(brier_cal), 'psi_score': float(psi_score),
    }
