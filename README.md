...
Dự án này đáp ứng đầy đủ các tiêu chuẩn đánh giá mô hình phân lớp (Classification) và hồi quy (Regression) với phương pháp kiểm định chéo (Cross Validation).

### 1. Bài toán Phân lớp (Probability of Default - PD)
Kết quả đánh giá trên tập Out-of-Fold bằng phương pháp **5-Fold Stratified Cross Validation** (k=5).

| Chỉ số (Metric) | Kết quả OOF (Champion: LightGBM) | Ý nghĩa |
| :--- | :---: | :--- |
| **ROC-AUC** | 0.7837 | Chỉ số đo lường khả năng phân tách Tốt/Xấu (Chuẩn ngành) |
| **Accuracy** | 0.7307 | Độ chính xác tổng thể |
| **Precision** | 0.1857 | Độ chính xác trên nhóm được dự báo Vỡ nợ |
| **Recall** | 0.6897 | Tỷ lệ bắt lỗi (phát hiện được 69% số ca vỡ nợ thực tế) |
| **F1-Score** | 0.2926 | Trung bình điều hòa của Precision và Recall |
| **Specificity** | 0.7343 | Tỷ lệ nhận diện đúng khách hàng Tốt |

> **Confusion Matrix (Ma trận nhầm lẫn):**  
> * True Negative (TN): 207,581 | False Positive (FP): 75,105
> * False Negative (FN): 7,702 | True Positive (TP): 17,123

<p align="center">
  <img src="reports/model_benchmark_roc.png" width="450" alt="ROC Curves Comparison">
  <img src="reports/calibration_curve.png" width="450" alt="Probability Calibration Curve">
</p>

### 2. Bài toán Hồi quy (Loss Given Default - LGD)
Mô hình Hồi quy LGD (LightGBM Regressor) được huấn luyện trên tập hồ sơ vỡ nợ (TARGET = 1) với phương pháp **5-Fold Cross Validation** (k=5) để dự phóng tỷ lệ tổn thất thực tế. 

Kết quả đánh giá (Mean ± Standard Deviation) qua 5 Folds:
* **MAE (Mean Absolute Error):** 0.0400 ± 0.0003
* **MSE (Mean Squared Error):** 0.0025 ± 0.0000
* **RMSE (Root Mean Squared Error):** 0.0501 ± 0.0004
* **R2 Score:** 0.0134 ± 0.0058

*Ghi chú: Mặc dù XGBoost có điểm số nhỉnh hơn một chút ở môi trường thử nghiệm đa mô hình, nhưng qua đánh giá chuyên sâu về tốc độ hội tụ, khả năng tối ưu hóa bộ nhớ (memory usage), và độ ổn định khi xử lý feature phân tán (sparse features) với quy mô dữ liệu cực lớn, chúng tôi quyết định chọn **LightGBM** làm mô hình cuối cùng (Champion) để triển khai lên Production.*
...
| **XGBoost** | 0.7869 | 0.5739 | 42.85% | 0.06584 | Contender |
| **LightGBM** | **0.7856** | **0.5711** | **42.83%** | **0.06598** | **Champion** |

*Ghi chú: Kết quả trên được trích xuất tự động từ luồng thực nghiệm K-Fold đa mô hình. Mặc dù XGBoost có điểm số nhỉnh hơn một chút ở môi trường thử nghiệm, nhưng qua đánh giá chuyên sâu về tốc độ hội tụ, khả năng tối ưu hóa bộ nhớ (memory usage), và độ ổn định khi xử lý feature phân tán (sparse features) với quy mô dữ liệu cực lớn, chúng tôi quyết định chọn **LightGBM** làm mô hình cuối cùng (Champion) để triển khai lên Production.*

### 📊 Đồ thị So sánh & Hiệu chuẩn Thực nghiệm
Hệ thống kết xuất các biểu đồ trực quan để kiểm chứng năng lực kỹ thuật:
