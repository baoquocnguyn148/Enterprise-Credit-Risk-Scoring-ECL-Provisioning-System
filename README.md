...
| Độ hoàn thiện | **8.0/10** - rất tốt cho portfolio/interview/demo, cần thêm CI/monitoring/fairness/reject inference để tiến tới production thật |
...
Hệ thống không chỉ tối ưu chỉ số học máy, mà còn chuyển xác suất vỡ nợ thành quyết định kinh doanh: phê duyệt, từ chối, review thủ công, thu hồi nợ sớm và trích lập dự phòng.
...
Dự án hiện có cả đánh giá học thuật lẫn đánh giá theo nghiệp vụ tín dụng. Phần PD là bài toán classification chính; phần LGD/EAD phục vụ chuyển đổi sang ECL theo IFRS 9.
...
**Ghi chú quan trọng về Logistic Regression:** Source code có Logistic Regression trong `src/modeling.py` dưới dạng baseline pipeline gồm median imputation, standard scaling và class-balanced logistic classifier. Vì vậy báo cáo hiện tại đã phản ánh đủ **5 thuật toán**, không còn thiếu LR.

**Ghi chú về champion/artifact:** bảng benchmark hiện cho CatBoost nhỉnh hơn rất nhẹ về OOF AUC, trong khi repo vẫn có cả `model_fold1-5.pkl`, `best_production_model.pkl` và legacy `lgbm_fold1-5.pkl`. Khi triển khai production thật, cần chốt lại champion registry để README, API và model artifacts đồng bộ tuyệt đối.
...
R2 thấp là hợp lý vì bộ dữ liệu Home Credit không có nhãn LGD thực tế. Repo mô phỏng `ACTUAL_LGD` theo logic Basel/segment và noise để minh họa workflow LGD; nếu dùng trong ngân hàng thật, cần thay bằng dữ liệu recovery/loss lịch sử.
...
LightGBM và CatBoost hiện là hai mô hình dẫn đầu rất sát nhau. LightGBM vẫn là lựa chọn có tính triển khai tốt vì tốc độ nhanh, footprint bộ nhớ thấp và phù hợp dữ liệu tabular/sparse sau feature engineering.

| Cơ chế | Ý nghĩa trong credit risk |
|---|---|
| Gradient Boosting Decision Trees | Mỗi cây mới sửa lỗi của các cây trước, giúp học tốt các quan hệ phi tuyến trong hồ sơ tín dụng |
...
Về mặt học thuật, Logistic Regression đóng vai trò baseline tuyến tính rõ ràng; RandomForest là benchmark bagging; XGBoost/CatBoost/LightGBM là nhóm gradient boosting mạnh hơn cho dữ liệu tabular.
...
| Kịch bản | Vai trò |
|---|---|
| Optimistic | Kiểm tra tác động khi môi trường kinh tế tốt hơn dự kiến |
...
**Bước 4 - Hyperparameter Optimization:** Optuna TPE sampler hỗ trợ tìm cấu hình tốt cho boosting model, giảm phụ thuộc vào trial thủ công.
...
### Đánh giá riêng phần Time Series: 7.5/10

Phần này có giá trị tốt cho **scenario analytics và dashboard storytelling**, nhưng chưa phải time-series forecasting đúng nghĩa vì dataset Home Credit không có lịch sử performance/default theo tháng thật. Source đã xử lý khá trung thực bằng cách đánh dấu chất lượng ngày là `proxy`, tức ngày được suy luận/mô phỏng để phục vụ phân tích. Để nâng lên 9/10, cần dữ liệu snapshot theo tháng, delinquency history, payment performance và mô hình forecast thật như survival analysis, hazard model hoặc roll-rate Markov chain.
...
Metrics được track gồm AUC, Gini, KS, Brier, calibration, PSI và các artifact liên quan. Đây là nền tảng tốt cho model governance, dù vẫn cần CI/CD, model registry policy và approval workflow nếu đưa vào production thật.
...
Điểm đã cải thiện trong source:
...
| CORS cấu hình bằng env | Phù hợp hơn với triển khai thật |
...
Phần dashboard tác nghiệp chuyển hóa kết quả pipeline thành giao diện dễ đọc cho business user. Nguồn dữ liệu trung tâm là `data/results_df.parquet` / `data/results_ifrs9.parquet`, chứa ID khách hàng, nhãn thực tế, PD đã hiệu chuẩn, risk tier, EAD, LGD, ECL và các feature quan trọng.
...
| ECL & Default Rate by DTI | Chứng minh DTI cao làm ECL tăng mạnh |
...
Hệ thống có `schema_sqlserver.sql` và bộ SQL/report phục vụ khai thác scoring output trong môi trường Data Warehouse. Đây là phần rất quan trọng vì nó chứng minh project không dừng ở notebook mà có khả năng phục vụ các phòng ban nghiệp vụ.
...
**Chiến lược:** Tìm khách hàng thu nhập tốt, rủi ro thấp để cross-sell/upsell; loại nhóm High Risk khỏi paid audience để giảm chi phí.
